...
export const Chart: FC<Props> = ({ chartTitle }) => {
  ...
  useEffect(() => {
    ...
    chart.setOption({
      ...
      series: [
        ...
        },
        {
          ...
          data: rawData.parkingApron.data.map(function (item: any, index) {
    ...
    initDrag(chart);
    // @ts-ignore
  }, [chartTitle, selectedAutoService.address]);

  const initDrag = (eChart) => {
    ...
    eChart.on('mousedown', (param) => {
      ...
      const arr = rawData.flight.dimensions.map((dimension: string, index: number) => ({
        dimension,
    ...
    const updateRawData = (params) => {
      const flightData = rawData.flight.data;
      ...
      for (let i = 0; i < flightData.length; i++) {
        ...
        if (
          (dataItem !== movingItem &&
            ...
            dropRecord.timeArrival < dataItem[DIM_TIME_DEPARTURE] &&
            dropRecord.timeDeparture > dataItem[DIM_TIME_ARRIVAL]) ||
          params.offsetY < 90 ||
          params.offsetX < 100 ||
      ...
      movingItem[DIM_TIME_ARRIVAL] = dropRecord.timeArrival;
      movingItem[DIM_TIME_DEPARTURE] = dropRecord.timeDeparture;
